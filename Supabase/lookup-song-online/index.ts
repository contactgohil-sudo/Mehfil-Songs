...
async function safeJson(url: string, init?: RequestInit, timeoutMs = 3500) {
...
async function fetchLyricsOvhCandidates(input: { query: string }) {
  const suggest = await safeJson(
    `https://api.lyrics.ovh/suggest/${encodeURIComponent(input.query)}`,
    undefined,
    2200
  );

  const rows = Array.isArray(suggest?.data) ? suggest.data.slice(0, 3) : [];

  const candidatePromises = rows.map(async (row: Record<string, unknown>) => {
    const title = String(row?.title || "").trim();
    const singer = String((row as { artist?: { name?: string } })?.artist?.name || "").trim();
    const movie = String((row as { album?: { title?: string } })?.album?.title || "").trim();
  ...
  return resolved.filter(Boolean) as Candidate[];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const evidencePromise = fetchTavilyEvidence(query).catch(() => []);

const [lrclibCandidates, ovhCandidates, evidence] = await Promise.all([
